...
function component(object) {
    ...
    {
        name: 'vk_scraper',
        title: 'VK Scraper (Парсер)',
        source: new VKVideoScraperSource(this, object), 
        search: true, // or false, depending on if it supports direct search from Lampa's input
        kp: false,    // Does it use Kinopoisk ID for search?
        imdb: false,  // Does it use IMDB ID for search?
        notice: 'Экспериментально! Скорее всего не работает из-за защиты от ботов.',
        disabled: false // or based on some condition, e.g., !Utils.isDebug()
    }];
